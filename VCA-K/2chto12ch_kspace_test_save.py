...
parser.add_argument("--ckpt", type=str, default='./exps/2sccto12gcc_cross_weight12_smooth_l1_to_85_epoch/checkpoint/0025.pth', help="Checkpoint path.")
...
def main(args):
    ...
    for i_batch, sample_batched in enumerate(tqdm(dataloader)):
        ...
        savemat('./results/input/kspace/{}_kspace.mat'.format(iiii), {'Img':ori_4coil_complex})
        # input: from kspace to image
        ori_4coil_complex_img = np.zeros(ori_4coil_complex.shape, dtype=np.complex64)
        for i in range(ori_4coil_complex.shape[-1]):
            ori_4coil_complex_img[:, :, i] = np.fft.ifft2(ori_4coil_complex[:, :, i])
        ori_4coil_complex_sos = np.sqrt(np.sum(np.abs((ori_4coil_complex_img)**2), axis=2))
        savemat('./results/input/sos/{}_sos.mat'.format(iiii), {'Img':ori_4coil_complex_sos})
        # ori
        ...
        # kspace to image
        savemat('./results/ori/kspace/{}_kspace.mat'.format(iiii), {'Img':ori_complex})
        ...
        savemat('./results/ori/sos/{}_sos.mat'.format(iiii), {'Img':ori_complex_sos})
        # back
        ...
        # kspace to image
        savemat('./results/for/kspace/{}_space.mat'.format(iiii), {'Img':pred_12_complex})
        ...
        pred_12_complex_sos = np.sqrt(np.sum(np.abs((pred_12_complex_img)**2), axis=2))
        savemat('./results/for/sos/{}_sos.mat'.format(iiii), {'Img':pred_12_complex_sos})
        ...
        print('psnr_backward:',psnr_decompress,'    ssim_backward:',ssim_decompress)
    ...
    print("ave_psnr_forward: %.10f || ave_psnr_backward:%.10f"%(ave_psnr_compress, ave_psnr_decompress))
    print("ave_ssim_forward: %.10f || ave_ssim_backward:%.10f"%(ave_ssim_compress, ave_ssim_decompress))
